---
title: "0da6 - 2021.11.07 - 00h00 - 01h00"
...
# Data 
```{r }
`0da6_7nov` <- read.csv2("~/Devellyn/traitement_data position/Firminy installation/Firminy installation/data/0da6_7nov.csv")

tagId <- unique(`0da6_7nov`$tag)
```
...
# Analyse des donnnées entre 00h00 et 01h00

```{r}
data
x <- intersect(which(`0da6_7nov`$date> "2021-11-07 00:00:00"), which(`0da6_7nov`$date< "2021-11-07 01:00:00"))
data <- `0da6_7nov`[x,]
...
# points <- plan+
#   geom_point(data = data, aes(measuredX, measuredY), size=0.1, color="red")+
#   # fFor animating the plot
#   labs(title = '2e8d: {frame_time}', x = 'X', y = 'Y') +
...
# xy <- cbind(as.numeric(data$measuredX), as.numeric(data$measuredY))
xy <- data
xy <- na.omit(xy)
...
write_xlsx(corr.data, 'stabilized_oda6_7nov_00h-01h.xlsx')
