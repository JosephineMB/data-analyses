---
title: "analyse 0da6 8nov21_8h-9h"
---
...
# Data 
```{r }
`0da6_8nov21` <- read.csv2("~/Devellyn/traitement_data position/Firminy installation/Firminy installation/data/0da6_8nov21.csv")

tagId <- unique(`0da6_8nov21`$tag)
```
...
# Analyse des donnnées du tag 0da6 le 8 novembre 2021 entre 08h00 et 10h00
...
The aim of this algoritm is to identify dense regions, which can be measured by the number of objects near a given point.

Two important parameters are needed for DBSCAN: the epsilon ("eps") and the minimum points ("MinPts"). The eps parameter defines the neighborhood radius around a point x. It is called the epsilon neighborhood of x. The MinPts parameter is the minimum number of neighbors in the "eps" radius.

Any point x in the data set, whose number of neighbors is greater than or equal to MinPts, is marked as a center point. We say that x is a boundary point, if the number of its neighbors is less than MinPts, but it belongs to the neighborhood of a central point z. Finally, if a point is neither a central point nor a boundary point, then it is called a noise or outlier point.

## Elbow method to calculate eps
The method proposed here consists in computing the distances between the k nearest neighbors in a matrix of points.

The idea is to calculate the average of the distances from each point to its k nearest neighbors. The value of k will be specified by the user and corresponds to MinPts.

Then, these k-distances are plotted in an increasing order. The objective is to determine the "knee", which corresponds to the optimal eps parameter.

A knee corresponds to a threshold where an abrupt change occurs along the k-distances curve.
